...
void struct_sptrsv_f(const TYPE *a, TYPE *x, const TYPE *b, int M, int N, int L, \
            ...
            for (c = 0; c < len; c+=vec)
            {
                ...
                for (u = cols - 1; u >= 0; u--)
                {
                    if (bid[u] != -1 && bneed[u])
                    {
                        ...
                        //if (id == 6) printf("id %d writing bid[%d] dest %d val %d\n", id, u, bid[u], count + recv_offset[u]);
...
void struct_sptrsv_entry(void *_info)
{
    struct_info info;
    CRTS_dma_get(&info, _info, sizeof(struct_info));

    struct_sptrsv_f(info.a, info.x, info.b, info.M, info.N, info.L, \
